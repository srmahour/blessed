...
function filterResources(resourceStore, doFilterResourcesWithEvents, eventStore, activeRange) {
    if (doFilterResourcesWithEvents) {
        ...
        tslib.__assign(hasEvents_1, computeAncestorHasEvents(hasEvents_1, resourceStore));
        return common.filterHash(resourceStore, function (resource, resourceId) { return hasEvents_1[resourceId]; });
    }
    return resourceStore;
}
function filterEventInstancesInRange(eventInstances, activeRange) {
    return common.filterHash(eventInstances, function (eventInstance) { return common.rangesIntersect(eventInstance.range, activeRange); });
}
...
function buildResourceEventUis(resourceStore) {
    return common.mapHash(resourceStore, function (resource) { return resource.ui; });
}
...
function fetchSource(source, fetchRange, context) {
    ...
    return tslib.__assign(tslib.__assign({}, source), { isFetching: true, latestFetchId: fetchId });
}
function receiveResponse(source, fetchId, fetchRange) {
    if (fetchId === source.latestFetchId) {
        return tslib.__assign(tslib.__assign({}, source), { isFetching: false, fetchRange: fetchRange });
    }
    return source;
}
...
function parseResource(raw, parentId, store, context) {
    ...
    var resource = {
        ...
        extendedProps: tslib.__assign(tslib.__assign({}, extra), refined.extendedProps),
    };
...
function addResource(existingStore, additions) {
    ...
    return tslib.__assign(tslib.__assign({}, existingStore), additions);
}
function removeResource(existingStore, resourceId) {
    var newStore = tslib.__assign({}, existingStore);
    delete newStore[resourceId];
    // promote children
    for (var childResourceId in newStore) { // a child, *maybe* but probably not
        if (newStore[childResourceId].parentId === resourceId) {
            newStore[childResourceId] = tslib.__assign(tslib.__assign({}, newStore[childResourceId]), { parentId: '' });
        }
    }
    return newStore;
}
function setResourceProp(existingStore, resourceId, name, value) {
    ...
    // TODO: sanitization
    if (existingResource) {
        return tslib.__assign(tslib.__assign({}, existingStore), (_a = {}, _a[resourceId] = tslib.__assign(tslib.__assign({}, existingResource), (_b = {}, _b[name] = value, _b)), _a));
    }
    return existingStore;
}
function setResourceExtendedProp(existingStore, resourceId, name, value) {
    ...
    var existingResource = existingStore[resourceId];
    if (existingResource) {
        return tslib.__assign(tslib.__assign({}, existingStore), (_a = {}, _a[resourceId] = tslib.__assign(tslib.__assign({}, existingResource), { extendedProps: tslib.__assign(tslib.__assign({}, existingResource.extendedProps), (_b = {}, _b[name] = value, _b)) }), _a));
    }
    return existingStore;
}
...
function reduceResourceEntityExpansions(expansions, action) {
    ...
    switch (action.type) {
        case 'SET_RESOURCE_ENTITY_EXPANDED':
            return tslib.__assign(tslib.__assign({}, expansions), (_a = {}, _a[action.id] = action.isExpanded, _a));
        default:
            return expansions;
    }
}
    ...
    ResourceApi.prototype.toPlainObject = function (settings) {
        ...
        if (Object.keys(internal.extendedProps).length) {
            if (settings.collapseExtendedProps) {
                tslib.__assign(res, internal.extendedProps);
            }
            else {
...
var ResourceSplitter = /** @class */ (function (_super) {
    tslib.__extends(ResourceSplitter, _super);
    function ResourceSplitter() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    ResourceSplitter.prototype.getKeyInfo = function (props) {
        return tslib.__assign({ '': {} }, props.resourceStore);
    };
...
function isPropsValidWithResources(combinedProps, context) {
    ...
    var sets = splitter.splitProps(tslib.__assign(tslib.__assign({}, combinedProps), { resourceStore: context.getCurrentData().resourceStore }));
    for (var resourceId in sets) {
        var props = sets[resourceId];
        // merge in event data from the non-resource segment
        if (resourceId && sets['']) { // current segment is not the non-resource one, and there IS a non-resource one
            props = tslib.__assign(tslib.__assign({}, props), { eventStore: common.mergeEventStores(sets[''].eventStore, props.eventStore), eventUiBases: tslib.__assign(tslib.__assign({}, sets[''].eventUiBases), props.eventUiBases) });
        }
        if (!common.isPropsValid(props, context, { resourceId: resourceId }, filterConfig.bind(null, resourceId))) {
            return false;
        }
    }
    return true;
...
function filterConfig(resourceId, config) {
    return tslib.__assign(tslib.__assign({}, config), { constraints: filterConstraints(resourceId, config.constraints) });
}
...
var ResourceCell = /** @class */ (function (_super) {
    tslib.__extends(ResourceCell, _super);
    function ResourceCell() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    ResourceCell.prototype.render = function () {
        var props = this.props;
        return (common.createElement(ResourceLabelRoot, { resource: props.resource, date: props.date }, function (elRef, customClassNames, dataAttrs, innerElRef, innerContent) { return (common.createElement("th", tslib.__assign({ ref: elRef, className: ['fc-col-header-cell', 'fc-resource'].concat(customClassNames).join(' '), colSpan: props.colSpan }, dataAttrs),
...
var ResourceDayHeader = /** @class */ (function (_super) {
    tslib.__extends(ResourceDayHeader, _super);
    ...
    ResourceDayHeader.prototype.renderResourceRow = function (resources, date) {
        var resourceCells = resources.map(function (resource) { return (common.createElement(ResourceCell, { key: resource.id, resource: resource, colSpan: 1, date: date })); });
    ...
    ResourceDayHeader.prototype.renderDateCell = function (date, dateFormat, todayRange, colSpan, resource, isSticky) {
        ...
        return props.datesRepDistinctDays ? (common.createElement(common.TableDateCell, { key: date.toISOString() + keyPostfix, date: date, dateProfile: props.dateProfile, todayRange: todayRange, colCnt: props.dates.length * props.resources.length, dayHeaderFormat: dateFormat, colSpan: colSpan, isSticky: isSticky, extraHookProps: extraHookProps, extraDataAttrs: extraDataAttrs })) : (common.createElement(common.TableDowCell // we can't leverage the pure-componentness becausae the extra* props are new every time :(
...
var ResourceDayTableModel = /** @class */ (function (_super) {
    tslib.__extends(ResourceDayTableModel, _super);
...
var DayResourceTableModel = /** @class */ (function (_super) {
    tslib.__extends(DayResourceTableModel, _super);
...
var VResourceJoiner = /** @class */ (function () {
    ...
    */
    VResourceJoiner.prototype.joinProps = function (propSets, resourceDayTable) {
        ...
        return {
            dateSelectionSegs: this.joinDateSelection.apply(this, tslib.__spreadArray([resourceDayTable], dateSelectionSets)),
            businessHourSegs: this.joinBusinessHours.apply(this, tslib.__spreadArray([resourceDayTable], businessHoursSets)),
            fgEventSegs: this.joinFgEvents.apply(this, tslib.__spreadArray([resourceDayTable], fgEventSets)),
            bgEventSegs: this.joinBgEvents.apply(this, tslib.__spreadArray([resourceDayTable], bgEventSets)),
            eventDrag: this.joinEventDrags.apply(this, tslib.__spreadArray([resourceDayTable], eventDrags)),
            eventResize: this.joinEventResizes.apply(this, tslib.__spreadArray([resourceDayTable], eventResizes)),
    ...
    VResourceJoiner.prototype.joinInteractions = function (resourceDayTable) {
        ...
        for (var i = 0; i < resourceCnt; i += 1) {
            ...
            if (interaction) {
                ...
                tslib.__assign(affectedInstances, interaction.affectedInstances);
...
var VResourceSplitter = /** @class */ (function (_super) {
    tslib.__extends(VResourceSplitter, _super);
    function VResourceSplitter() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    VResourceSplitter.prototype.getKeyInfo = function (props) {
        var resourceDayTableModel = props.resourceDayTableModel;
        var hash = common.mapHash(resourceDayTableModel.resourceIndex.indicesById, function (i) { return resourceDayTableModel.resources[i]; }); // :(
        hash[''] = {};
        return hash;
    };
...
function buildResourceFields(resource) {
    var obj = tslib.__assign(tslib.__assign(tslib.__assign({}, resource.extendedProps), resource.ui), resource);
    delete obj.ui;
